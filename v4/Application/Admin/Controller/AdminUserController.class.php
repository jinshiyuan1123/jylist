...
class AdminUserController extends AdminController {
    ...
    public function add(){
        if(!IS_POST){
        	...
        	$adminId = D('AdminUser')->order('user_id desc')->getField('user_id');
        	$mod = M('Users');
        	$id = $mod->order('id desc')->getField('id');
        	if($adminId>=$id){       	
        		$arr ['user_login'] = time().rand(1, 10000);
        		$arr ['idmd5'] = md5($log. C ( 'PWD_SALA' )."123456");
        	    $newId = $mod-> add($arr);
        	    $mod->where('id = '.$newId)->delete();
        		$_POST['user_id'] = $newId;	
        	}
